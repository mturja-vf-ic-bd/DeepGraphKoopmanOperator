...
from models.ScalableKNF2 import SKNF
from trainers.metrics import SMAPE
from trainers.varKNF_utils import train_epoch_koopman, eval_epoch_koopman, get_lr
...
def main(rank, world_size, input_dim, input_length,
         latent_dim, real_modes, complex_modes,
    ...
    model_name = (
            ...
            + f"_seed{seed}_jumps{jumps}_poly{num_poly}_"
              ...
              f"num{num_steps}__latdim{latent_dim}_rm={real_modes}"
              f"_cm={complex_modes}_RevIN{use_revin}_"
              f"insnorm{use_instancenorm}_stride{stride}_rank{dmdrank}"
    )
    print(model_name)
    model = SKNF(
        ...
        # Stride by which feature windows shift to right
        stride=stride
    ).to(rank)
    ...
    if mode == "train":
        for epoch in tqdm(range(last_epoch, num_epochs)):
            ...
            train_rmse, train_recon, train_pred, train_embedding, \
            train_embedding_lr, train_recon_lr = train_epoch_koopman(
                train_loader,
                model,
                loss_fun,
                optimizer,
                rank=rank)
            eval_rmse, _, _, _, _, _, _, eval_pred, eval_pred_lr, eval_recon_lr = eval_epoch_koopman(
                valid_loader,
                model, loss_fun,
                rank=rank)
            ...
            print(
                ...
                'Train Embedding: {:0.3f}, Train Embedding LR: {:0.3f}, Train Recon LR: {:0.3f} | '
                'Valid RMSE: {:0.3f}, Valid Pred: {:0.3f}, Valid Embedding LR: {:0.3f}, Valid Recon LR: {:0.3f}'
                .format(epoch + 1, epoch_time / 60, train_rmse,
                        train_recon, train_pred, train_embedding, train_embedding_lr, train_recon_lr,
                        eval_rmse, eval_pred, eval_pred_lr, eval_recon_lr),
                flush=True)
    elif mode == "test":
        print(f"Test set length: {len(test_loader)}")
        test_rmse, test_preds, test_tgts, \
        test_recon, test_recon_true, test_dmd_modes, \
        test_dmd_vals, test_pred_rmse, _, _ = eval_epoch_koopman(
...
def run_ddp_training():
    ...
    parser.add_argument("-m", "--max_epochs", nargs="?", type=int, default=200)
    parser.add_argument("--lr", default=0.0003, type=float, help="learning rate")
    parser.add_argument("--mode", choices=["train", "test"], default="train")
    parser.add_argument("--write_dir", type=str, default="log/sKNF")
    ...
    parser.add_argument("--n_complex_modes", type=int, default=62)
    parser.add_argument("--dmd_rank", type=int, default=64)
    ...
    parser.add_argument("--num_poly", type=int, default=5)
    parser.add_argument("--num_sins", type=int, default=24)
    ...
    parser.add_argument("--data_freq", type=str, default=None)
    parser.set_defaults(
        use_revin=False,
    ...
    mp.spawn(main,
             args=(world_size, args.input_dim, args.input_length,
                   ...
                   args.n_complex_modes, args.train_output_length, args.test_output_length,
                   ...
                   args.hidden_dim, args.num_layers,
                   args.num_feats, args.dropout_rate, args.min_epochs, args.mode, args.stride, args.data_freq, args.dmd_rank),
