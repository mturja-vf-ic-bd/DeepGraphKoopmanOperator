...
from dataloaders.MegaTrawl import MegaTrawlDataset, HCPTaskfMRIDataset, M4Dataset, CrytosDataset
from models.SparseKNF import SparseKNF
...
def main(rank, world_size, input_dim, input_length,
         ...
         add_global_operator, batch_size, num_epochs, learning_rate,
         ...
         mode, stride, freq, dmd_rank, sbs_channel, sbs_nlayer):
    ...
    data_dir = os.path.join(CONSTANTS.CODEDIR, data_dir[dataset_name])
    direc = os.path.join(data_dir, "train.npy")
    direc_test = os.path.join(data_dir, "test.npy")
    eval_metric = metric_dict[dataset_name]
    ...
    if dataset_name == "megatrawl":
        ...
        test_set = MegaTrawlDataset(
            ...
            jumps=jumps,
            mode="test")
    elif dataset_name == "M4" or dataset_name == "Cryptos":
    ...
    model_name = (
            ...
            + f"_seed{seed}_jumps{jumps}_poly{num_poly}_"
              ...
              f"insnorm{use_instancenorm}_"
              f"globalK{add_global_operator}"
              f"_stride{stride}_rank{dmd_rank}_sbsc{sbs_channel}"
    )
    print(model_name)
    model = SparseKNF(
        ...
        sbs_channel=sbs_channel,
        sbs_stride=2,
        sbs_nlayer=sbs_nlayer,
        # number of feature
        num_feats=num_feats,
    ...
    if mode == "train":
        for epoch in tqdm(range(last_epoch, num_epochs)):
            ...
            train_rmse, train_recon, train_pred, train_embedding = train_epoch_koopman(
                ...
                optimizer,
                rank=rank)
            eval_rmse, eval_preds, eval_tgts, _, _, _, _, eval_pred_loss = eval_epoch_koopman(
                valid_loader,
                model, loss_fun,
                rank=rank)
            eval_smape = np.abs(eval_preds - eval_tgts) * 200 / (
                        np.abs(eval_preds) + np.abs(eval_tgts))
            eval_smape = np.round(np.mean(eval_smape), 3)
            if eval_smape < best_eval_smape and rank == 0:
                best_eval_smape = eval_smape
                best_model = model
            ...
            print(
                ...
                'Train Embedding: {:0.3f} | Valid RMSE: {:0.3f}, Valid Pred: {:0.3f}, Valid smape: {:0.3f}'
                .format(epoch + 1, epoch_time / 60, train_rmse,
                        train_recon, train_pred, train_embedding,
                        eval_rmse, eval_pred_loss, eval_smape),
                flush=True)
    elif mode == "test":
        ...
        if dataset_name == "M4":
            ...
            test_tgts = (test_tgts * test_set.ts_stds.reshape(
                -1, 1, 1)) + test_set.ts_means.reshape(-1, 1, 1)
        elif dataset_name == "Cryptos":
            test_preds = test_preds.reshape(
                14, -1, test_output_length,
                8)  # 14 stocks x num_samples x #steps x 8 features
            test_tgts = test_tgts.reshape(14, -1, test_output_length, 8)
        ...
        print(f"{str(eval_metric)} scores:", eval_score)
        if rank == 0:
            torch.save(
                {
...
def run_ddp_training():
    ...
    parser.add_argument("--sbs_channel", type=int, default=128)
    ...
    parser.add_argument("--sbs_nlayer", type=int, default=2)
    parser.set_defaults(
        use_revin=False,
    ...
    mp.spawn(main,
             args=(world_size, args.input_dim, args.input_length,
                   ...
                   args.add_global_operator, args.batch_size, args.max_epochs, args.lr,
                   ...
                   args.stride, args.data_freq, args.dmd_rank, args.sbs_channel, args.sbs_nlayer),
